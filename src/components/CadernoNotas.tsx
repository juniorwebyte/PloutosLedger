...
export default function CadernoNotas({ onBackToLanding }: CadernoNotasProps) {
  ...
  const isAuthenticated = localStorage.getItem('cadernoAuthenticated') === 'true' || !!localStorage.getItem('auth_token');

  // Se não estiver autenticado, não renderizar o componente
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">Acesso Negado</h1>
          <p className="text-gray-600 mb-6">Você precisa fazer login para acessar o Caderno de Notas.</p>
          {onBackToLanding && (
            <button
            ...
            <tbody>
              ${filteredNotas.map(nota => `
                <tr>
                  ...
                  <td>${new Date(nota.vencimento).toLocaleDateString('pt-BR')}</td>
